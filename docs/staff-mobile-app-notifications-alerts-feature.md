...
The design challenge is not *whether* to notify but *how* to notify without drowning staff. The value is in **relevance and timeliness**: filterable, prioritized, per-role alerts with history — plus critical alerts that bypass Do-Not-Disturb — so staff trust the stream and act on it.

Vendors ship this as a core mobile capability: Cloudbeds (filterable real-time push with history), SiteMinder Little Hotelier (instant booking notifications), and eZee.
...
| 2 | **Filterable & prioritized** | Filter by type and department; priority tiers so critical rises to the top. |
| 3 | **Notification history** | A searchable log of past alerts — nothing lost if missed live. |
| 4 | **Per-user preferences & quiet hours** | Each user tunes what they receive and when. |
| 5 | **Critical/emergency bypass** | Life-safety, VIP-arrival, and escalation alerts override Do-Not-Disturb. |
| 6 | **Tap-through to action** | Every alert deep-links to the relevant reservation, task, room, or thread. |
| 7 | **In-app inbox fallback** | If push fails, alerts still appear in an in-app inbox. |

### 2.2 In scope (this release)
Real-time push for bookings/cancellations/messages/tasks/SLA, filtering + priority tiers, notification history, per-user preferences + quiet hours, critical bypass, deep-link tap-through, in-app inbox.
...
- Users have per-role assignments to scope notifications.
...
### 3.2 Preferences, quiet hours & critical bypass
...
Filterable, priority-tiered staff notifications with history are not standard among BD vendors. Atrium can lead locally.
...
2. **Fewer missed-event costs.** Timely alerts prevent the comps, resell losses, and escalations that missed events cause.
3. **Preserved attention.** Filtering and quiet hours reduce notification fatigue that otherwise wastes attention and hides real signals.
...
- Real-time push for bookings/cancellations/messages/tasks
